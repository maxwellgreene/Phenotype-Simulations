...
runSimAllEnvGenLin <- function(environment,nRun=2,nEach=2,nGen=3)
{
  ...
  colony <- expand.grid(early= seq(1.0,1.0,length.out = nEach),
                        middle=seq(1.0,1.0,length.out = nEach),
                        late=  seq(1.0,1.0,length.out = nEach),
                        scale= seq(1.0,1.0,length.out = nEach));
...
runSimAllDen <- function(nRun,nEach,range,env=c(.6,.7,.8,.8),col=c(.5,.5,.75,.75), which = rep(1,8), return = "vector")
{
